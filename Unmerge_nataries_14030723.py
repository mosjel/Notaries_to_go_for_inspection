...
try: 
    ...
    file_path = dir_name+"\\"+"total_1_R_1403_04_20 روحی _Beruz_14030610.xlsx"
    ...
    df_merged = df.groupby(df.columns[[1,2,3,4,5,7,8,9]].tolist(),dropna=False)[df.columns[6]].apply(lambda x: ('*'.join(x))).reset_index()
    ...
    # column_list=df_merged.columns[0:6]
    # print(column_list)
    # input("KKKKKKKKKKKKKKKKKKKKK")
    # input("HHhhhhhhhggggggggggggg")
    # print(df_merged.shape)
    # input("gigili")


    last_valid_index=df_merged.iloc[:,8].last_valid_index()
    print(last_valid_index)
    print(df_merged.shape)
    input()
    df_merged.insert(0, "ردیف", list(range(1,last_valid_index+2)), True)
    df_merged=df_merged.iloc[:,[0,1,2,3,4,5,9,6,7,8]]
    ...
    output_file_path=dir_name+"\\"+"notaries_information_corrected.xlsx"
