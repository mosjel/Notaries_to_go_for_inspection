...
notary_not_mistake_index=(np.isin(visited_notaries,notary_list))
...
df_merged_correct_notary_number=df_merged[notary_not_mistake_index].copy()

print(df_merged.shape)
print(df_merged_correct_notary_number.shape)
print(df_merged.head())
print(df_merged_correct_notary_number.head())

df_mistake_risk["شماره دفترخانه اشتباه"]="Mistake"


df_error=df_merged_correct_notary_number[df_merged_correct_notary_number[df_merged_correct_notary_number.columns[8]]=="Error"]
df_na=df_merged_correct_notary_number[df_merged_correct_notary_number[df_merged_correct_notary_number.columns[8]].isnull()].copy()
...
df_merged_correct_notary_number[df_merged_correct_notary_number.columns[8]]=pd.to_datetime(df_merged_correct_notary_number[df_merged_correct_notary_number.columns[8]],errors="coerce").dt.date

df_merged_correct_notary_number.dropna(subset=[df_merged_correct_notary_number.columns[8]], inplace=True)
...
df_dates=df_merged_correct_notary_number[df_merged_correct_notary_number.groupby(df_merged_correct_notary_number.columns[4],dropna=False)[df_merged_correct_notary_number.columns[8]].transform("max")==df_merged_correct_notary_number[df_merged_correct_notary_number.columns[8]]]

# df_dates=df_merged_correct_notary_number.groupby(df_merged_correct_notary_number.columns[4],dropna=False)[df_merged_correct_notary_number.columns[8]].transform("max")
