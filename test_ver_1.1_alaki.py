...
grouped= df.groupby(df.columns[[0,1,2,3,4,5]].tolist(),dropna=False).apply(lambda x:x.index.min()+2)
...
# df_na=df_merged[df_merged[df_merged.columns[8]].isnull()]
...
# df_dates_violated=df_dates[df_dates[df_dates.columns[5]]!="عدم تخلف"].sort_values(df_dates.columns[8],ascending=True)
# df_dates_not_violated=df_dates[df_dates[df_dates.columns[5]]=="عدم تخلف"].sort_values(df_dates.columns[8],ascending=True)

# print(df_dates_violated.shape)
# print("------------------------------------------------")
# print(df_dates_not_violated.shape)
...
# #         for cell in row:
# #             if cell.value in ["Error", "Mistake Risk"]:
# #                 # cell.fill = red_fill
